...
def downloadOrderResult(
    ...
    if not os.path.exists(download_path):
        ...
        # 获取transfer_result结果中content和orderResult的字符串内容
...
def extractValues(data, is_pinyin, style=Style.FIRST_LETTER):
    w_values = []
    # 如果data是一个列表，遍历列表中的每个元素，并递归调用extractValues函数，将返回的值扩展到w_values列表中
    if isinstance(data, list):
        for item in data:
            w_values.extend(extractValues(item, is_pinyin, style))
    # 如果data是一个字典，遍历字典中的每个键值对
    elif isinstance(data, dict):
        for key, value in data.items():
            if key == "w":
                # 是否将中文转成拼音首字母，结果用于音频切割，is_pinyin = True 表示转成汉字拼音的首字母
                if is_pinyin:
                    w_values.append(gbkXfrFstLetter(value))
                else:
                    w_values.append(value)
            else:
                w_values.extend(extractValues(value, is_pinyin))

    return w_values

# getTransferResult方法：从给定的JSON文件中获取转写结果，并根据需要将中文转换为拼音首字母
def getTransferResult(json_path, is_pinyin=False, style=Style.FIRST_LETTER):
    # 使用open函数打开JSON文件，读取其中的内容，并将其解析为Python对象
    ...
    for element in transfer_json:
        ...
        sentence_ws = sentence["st"]["rt"]
        w_values = extractValues(sentence_ws, is_pinyin=is_pinyin, style=style)
        concatenated_string = "".join(w_values)
        w_list.append(concatenated_string)
    transfer_result = "".join(w_list)
    jsonfile_name = os.path.basename(json_path)
    print(jsonfile_name, "的转写结果为", transfer_result)
...
if __name__ == '__main__':
    result_json_path = downloadOrderResult(upload_file_path=r'audio/qilai.mp3')
    w_str_result = getTransferResult(result_json_path, is_pinyin=False)